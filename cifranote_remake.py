...
class CifraNoteApp(MDApp):
    ...
    def on_start(self):
        ...
        self.tabs.add_widget(Tab(title="Nota Geral"))
    ...
    def create_new_note(self, title):
        ...
        elif title in self.notes: Snackbar(text="Nota já existente!").open()
        else:
    ...
    def rename_note(self, title):
        if title == "": Snackbar(text="Nome vazio!").open()
            ...
            self.notes.pop(self.notes.index(self.tabs.get_current_tab().title))
            self.notes.append(title)
